...
# =============================================================================
# 
    def glove_emb(self,glove_file_location, emb_size = emb_size, input_lengths = input_lengths, word_index = vocab.w2i):
        emb_index = {}
        with open(glove_file_location) as f:
            for line in f:
                values = line.split()
                word = values[0]
        ...
        emb = np.zeros((input_lengths, emb_size))
...
# =============================================================================
    ...
    def forward(self, input_tensor, input_lengths=None, init_hidden=None):
        ...
        emb = self.glove_emb(input_tensor)
...
class UniLSTMModel(RNNModule):
    ...
    def forward(self, input_tensor, input_lengths=None, init_hidden=None):
        ...
        h_n = self.get_last_hidden(rec_hidden)
...
class BiLSTMModel(UniLSTMModel):
    ...
    def forward(self, input_tensor, input_lengths=None, init_hidden=None):
        ...
        # Extract the last hidden vector
        h_n = self.get_last_hidden(rec_hidden,bidirect = True)
        # Learn the hidden representation
        fc_1 = self.fc_layer(h_n)
        # Dropout on the last layer
        h_n_drop = self.dropout_layer(fc_1)
        # Calculate un-nomalized scores
        decoded_scores = self.fc_layer(h_n_drop)
        # YOUR CODE ENDS HERE
        #######################
        return decoded_scores, rec_hidden, rec_output
