...
#         
        ...
        doc_lengths = [len(doc_ids)]
        doc_tensor = Data2tensor.idx2tensor([doc_ids])
        doc_lengths_tensor = Data2tensor.idx2tensor(doc_lengths)
        
# =============================================================================

        decoded_scor, rec_hid, rec_out = self.model(doc_tensor, doc_lengths_tensor)   
        label_prob, label_pred = self.model.inference(decoded_scor, k=5)        
        # label_tensor = Data2tensor.idx2tensor(label, device)
        last_pre = np.squeeze(label_pred.detach().numpy())
        label_pro = np.squeeze(label_prob.detach().numpy())
        dict1 = self.args.vocab.l2i
